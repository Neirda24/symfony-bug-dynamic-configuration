...
class EnumNodeBuilder extends NodeBuilder implements NodeBuilderInterface
{
    ...
    public function create(): EnumNodeDefinition
    {
        ...
        if (\count($this->values) === 0 && null === $this->callback) {
            throw new LogicException(\sprintf('You must choose between "values" or "callback" to provide any choice for "%s".', $this->name));
        }
        ...
        if (\count($this->values) > 0) {
            ...
            if (\is_string($this->callback)) {
                if (is_a($this->callback, \BackedEnum::class, true) === false) {
                    throw new LogicException('The class from the "callback" option is not a valid class "\BackedEnum"');
                }
        ...
        if (null !== $this->defaultValue) {
            if (!\in_array($this->defaultValue, $values, true)) {
                throw new LogicException(\sprintf('The default value "%s" is not part of the configured values "%s".', $this->defaultValue, implode(', ', $values)));
            }
        }
